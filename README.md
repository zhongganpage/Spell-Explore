...
[![Project graph visualization](docs/graph-visual.png)](https://zhongganpage.github.io/Spell-Explore/)
...
- **Formalizer** — Lean formalization. Lint-passed reports are decomposed into fragments; the working swarm (~4, unrelated to the lean code runner's swarm) writes per-fragment files; the **lean code runner** merges them into the single qmd file, plans the verification jobs in advance and dispatches them to its own swarm (whatever number the plan requires, at most 3). Green pieces are locked in the qmd, their lean code enters the reliable idea set with the `[Formalized]` marker; the Integrator owns the graph — `formalizer/Integrator/ITG.lean` tracks assumption nodes, green edges, `[Hired]` flags (only there, never in single.lean), and the goal node, and the integration report is the milestone source.
- **Integrator** — the fifth subcoordinator. Owns the dependency graph as `formalizer/Integrator/ITG.lean` plus the integration report (`formalizer/Integrator/integration-report.md`, the milestone source); runs two ephemeral workers — worker-1 (45-min primary proof writer) and worker-2 (15-min secondary lean-run/fix) — under a stable ID across rounds; writes `formalizer/hireable-registry.md` and `formalizer/connection-proofs.md`, and is the second writer of `dossier/idea-pool/reliable-idea-set/`.
...
- The project reaches a milestone only on full consensus (3/3 swarm + 3/3 BCD) with the accepted routes achieving the locked goal — operationally, the goal node reachable per the integration report from `[Formalized]`/`[Hired]` assumptions. Two consecutive rounds of total rejection (0/3, 0/3) trigger the Coordinator's steering report: split the goal, nominate pairings, or pause.
...
The Lean formalization pipeline: lint-passed reports → decompose → working swarm → lean code runner (plans and dispatches to its own swarm) → reliable idea set, ITG.lean + integration report, fragment region.
