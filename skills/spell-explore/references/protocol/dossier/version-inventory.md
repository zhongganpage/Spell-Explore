...
- the Formalizer's single qmd file (there is only one): every update is a new version; whenever a qmd piece is lean-green, the lean code runner locks that piece in the qmd file (green pieces are locked in place, never removed except by the runner's retraction duty — a GREEN record found false is retracted, record + comment) and places the corresponding lean code in the reliable idea set — a new entry, with its version, carrying the [Formalized] marker.
...
| qmd file update | single.qmd | v1 | v1 → … | Formalizer / lean code runner | any round | green pieces LOCKED in place; a false GREEN record retracted (record + comment) |
