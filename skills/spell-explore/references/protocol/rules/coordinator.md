...
- the Coordinator asks the user for the source-repository path — the local clone of the Spell-Explore repository that holds the canonical protocol — records it in runtime/coordinator-state.md (`protocol-src: <path>`) and in the dossier, and runs the protocol update against the remote: the repository's scripts/update-skill.sh --check probes the origin (a bounded read, not counted in the budget) and, when the local clone is behind the published protocol, applies it with the repository's scripts/update-skill.sh — fast-forwarding the clone (git pull --ff-only, never merge or reset; refused when the clone has uncommitted tracked changes) and syncing into the installed copy. then runs the protocol-integrity check: the repository's scripts/check-skill.sh with that path (a bounded read, not counted in the budget). when the installed protocol differs from the repository's, the Coordinator aligns the installed copy with the repository — running the repository's scripts/sync-skill.sh, re-running the check to verify it is clean, recording the aligned files in the dossier — and surfaces the drift and the alignment to the user;
...
### process hygiene — scoped searches, no orphaned children

adopted from the round-7 orphaned-grep incident (dossier/incident-report-orphan-grep-2026-08-13.md): two unscoped `grep` crawls of the user's home directory outlived their owning agents and streamed temp-file names that read as a download. the Coordinator enforces on itself, the subcoordinators, and every worker:

- **scope every search to the project root** — Grep/rg/Bash searches run with the project root as the search path, or with `--exclude-dir`/glob exclusions for the large non-project trees (AppData, WeChat Files, Downloads, .cache, .local, node_modules). a home-directory recursive scan (`/c/Users/<user>` or `C:\Users\<user>`) is a protocol violation: the Coordinator records it in the dossier and the round-start preflight checks no such scan is running.
- **label long-running scans** — a long-running local search prints its scope line up front ("scanning <dir>") and reports throttled/aggregated progress ("N files scanned, M matches"), never a bare stream of hash-like temp names that visually mimics a transfer list; the Coordinator corrects an agent that emits raw crawl output.
- **reap children at close** — an agent whose turn or phase closes leaves no live subprocess behind: the close procedure (the Coordinator's own turn discipline below; each subcoordinator's lifecycle close per rules/worker-lifespans.md) enumerates and terminates the agent's live child processes, or the shell runs with job control so the whole process tree dies with the agent. a closed agent's orphaned children are a violation and are recorded.
- **never silently reuse the upstream name** — a window/terminal a subagent can populate prefixes a bare tool name with the local scope ("git-bash grep (local scan)"), so an unattributed output stream is attributable on sight and never reads as the Spell-Explore upstream (`zhongganpage`) doing work.
