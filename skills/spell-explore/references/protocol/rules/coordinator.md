...
- the Producer turns pairs of fresh summaries into idea reports and then into routes;
...
## 5. The 133-minute timeline — announce, timestamp, cut, close

Each round has a hard budget of 2 hours and 13 minutes (133 minutes). Round timing is
mandated:
...
| 45–58 | hygiene linter + examine worker | linter layer 1 (mechanical) ≈2 min, linter layer 2 (the format + assumptions, implications and grouping) ≈6 min, then the examine worker (cap 5 min) |
| 58–98 | Selector panel | workerA lists the evidence points by 73 min; workerB/C/D review from 58 min and pivot to the list when it arrives; exchange reports 88–98 min, and each writes its review summaries |
| 98–113 | PI rebuts and modifies the route; promoter in parallel | the promoter writes its nearest true version note in the same window |
| 113–133 | the swarm decides | decision swarm (20 min) + resumed BCD reviewers (20 min) |
...
— the same rule as the 10-minute lemma cut — and the round closes atomically at 133
min even if a phase is mid-flight. These windows are the binding per-phase limits;
changing any of them means the 133-minute budget no longer holds. A round close never
...
queued route reviews and the Producer processes queued summary pairs.
...
there are stale documents (summaries / reports / routes) from the other
subcoordinators.
...
each fresh route as it becomes ready, and the 133-minute budget is the only bound. In
practice two route reviews fit per round (the Selector runs two panels at a time,
58–98; the two PI rebuttals run in parallel, 98–113; the two decision swarms run
together in the same window, 113–133, as background workers), so a third route's
...
- premature kills: stale or unaccepted items whose fragments (archived in the
  fragment region of the idea pool) later end up inside an accepted route, and
  whether they were killed by evidence or by opinion;
...
(0/9 swarm and 0/3 BCD), the Coordinator produces a steering report: the distribution
...
The acceptance numbers rank the quality of an accepted route: full consensus (9/9 +
3/3) is the strongest, lower counts are accepted but weaker.

Milestone: all 9 swarm workers and all 3 BCD reviewers accept, and the accepted routes
together achieve the locked goal. The operational reading of "achieve the locked
goal" is the goal node of the dependency tree reachable from the [Formalized] or
[Hired] assumptions.

When the project reaches a milestone, the Coordinator writes a report about it in
PDF, called the manuscript (locked name). The manuscript carries a version.
...
- question-routes.md — the main question and, as each route is accepted, the abstract
  ...
  accepted-route abstracts, and it carries a version;
...
is updated as a new version; nothing is deleted, everything is versioned.
...
by the 133-minute budget, and a round close never cuts its swarm.
