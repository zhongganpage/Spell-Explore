...
every round ends with a single atomic round close written in one pass, at 133 minutes, even if a
...
- the round clock: the round-1 setup — the Coordinator asking the user for the rough idea and
  ...
  the budget. the round itself is 133 minutes.
...
- round start: `<announced timestamp, written in the dossier before any agent spawned>` ·
  round end: `<133-min boundary timestamp>`
...
- accepted: `<title + version, with the acceptance counts — swarm x/9, BCD x/3. acceptance =
  ≥2/3 of the swarm workers AND ≥2/3 of the BCD reviewers; the counts rank the route — full
  consensus (9/9 + 3/3) is the strongest>`
- unaccepted: `<title + version, with the counts and the failure reason>`
- carried over: `<queued route reviews and queued summary pairs that carry to the next round —
...
the windows are the binding per-phase limits of the 133-minute timeline (critical path, one
route):
...
- 45–58 the hygiene linter and the examine worker decide sufficiency — layer 1 (mechanical)
  ≈2 min, layer 2 (the format + assumptions, implications and grouping) ≈6 min, then the
  examine worker (cap 5 min);
- 58–98 the Selector's panel — workerA lists the evidence points by 73 min, workerB/C/D review
  from 58 min and pivot to the list when it arrives, exchange reports 88–98;
- 98–113 the PI rebuts and modifies the route, with the promoter writing its nearest true
  version note in the same window;
- 113–133 the swarm decides, with the resumed BCD reviewers voting alongside.

off the critical path and in the background: the Creator's second phase, the Formalizer (not
bound by the 133-minute budget — a round close never cuts the swarm), and any additional
...
- the round closes atomically at 133 min in one pass, even when a phase is mid-flight; the
...
- when the round reaches a milestone — all 9 swarm workers and all 3 BCD reviewers accept, and
